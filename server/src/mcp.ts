...
const DEFAULT_PLATFORM_EXAMPLE: PlatformExample = {
  prefix: "EPIC",
...
function getPlatformExample(platform: string | undefined): PlatformExample {
  if (!platform) return DEFAULT_PLATFORM_EXAMPLE;
  return PLATFORM_EXAMPLES[platform] ?? DEFAULT_PLATFORM_EXAMPLE;
}

function ensureDocsScaffold(workspace: string, config: ServerConfig): string[] {
...
export function createMcpServer(overrides?: {
  ...
  if (overrides?.platform) process.env.SDLC_PLATFORM = overrides.platform;
